...
public class WitheredSoulWolfVariant extends WolfVariant {
    ...
    public WitheredSoulWolfVariant(String name) {
        super(
            ...
            .glowingOverlay(
                ...
                Resources.getDTNWolfTexture("variants/wolf_withered_soul_wild_overlay")
            )
        );
    }
    ...
    public void tickWolf(DTNWolf wolf) {
        ...
        if (wolf.getRandom().nextInt(3) == 0) {
            SimpleParticleType type = ParticleTypes.SOUL_FIRE_FLAME;    
            if (type != null) {
                double d0 = (double)wolf.getX() + RandomUtil.nextFloatRemapped(random) * (wolf.getBbWidth()/2);
                double d1 = (double)wolf.getY() + wolf.getBbHeight();
